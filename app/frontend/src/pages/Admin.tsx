    ...
    <div className="p-4">
      ...
      <form onSubmit={submit} className="space-y-3 max-w-lg">
        <div>
          ...
          <input value={deviceId} onChange={(e) => setDeviceId(e.target.value)} className="w-full border rounded px-2 py-1" />
        </div>
        <div>
        ...
        <div>
          <button disabled={loading} className="px-3 py-1 rounded bg-blue-600 text-white">{loading ? 'Saving...' : 'Register Device'}</button>
        </div>
      </form>

      {result && (
        <div className="mt-6 bg-white border rounded p-3 max-w-lg">
          <div className="text-sm text-gray-700">Device registered:</div>
          <div className="font-mono mt-2">{result.device_id}</div>
          ...
          {result.mqtt_key && (
            <div className="mt-3">
              ...
              <div className="font-mono bg-gray-100 p-2 rounded mt-1">{result.mqtt_key}</div>
